...
to_strataCrossN <- function (.strataPctList, .namesList, N) {
  ...
  if (length(.strataPctList) != length(.namesList))
    stop("`strataPctList` and `namesList`\u9577\u5ea6\u4e0d\u4e00\u81f4",
         call. = FALSE)
  if (!identical(setdiff(names(.strataPctList), names(.namesList)), character(0)))
    stop("names of `strataPctList` and `namesList` must be identical",
         call. = FALSE)
  ...
  .strataPctList <- lapply(.strataPctList,
                           function(x) {
                             x / sum(x)
                           })
  ...
  for (i in seq_along(.strataPctList)) {
    if (i == 1) {
      strataCrossArr <- .strataPctList[[1]]
    }
    else {
      strataCrossArr <- outer(strataCrossArr, .strataPctList[[i]])
    }
  }
  ...
  ## 抽樣陣列
  N <- as.integer(N)
  strataCrossN <- round(strataCrossArr * N, 0)
  # sum(strataCrossN)
  # plyr::adply(strataCrossN, c(1,2,3))
...
{
  ## 實抽樣本expanded
  strataCrossN.expanded <- .strataCrossN_flat[rep(row.names(.strataCrossN_flat), .strataCrossN_flat$Freq),
                                              1:length(.strataCrossN_flat)]
  
  strataCrossN.expanded <-
    strataCrossN.expanded[-length(strataCrossN.expanded)]
  ...
  probList <-
    c(probList[1], lapply(probList[-1], format, digits = 3, nsmall = 3))
...
renew_sampleN <-
  function (.strataCrossN_flat, outputfile = "R\u5be6\u62bd\u6a23\u672ctemp.csv") {
    ## 實抽樣本從txt重讀
...
compare_sample_size <- function (.data,
  ...
  if (!identical(setdiff(names(.vars), names(.namesList)), character(0)))
    stop("names of `.vars` and `.namesList` must be identical", call. =
           FALSE)
  if (!identical(setdiff(names(.strataCrossN_flat)[names(.strataCrossN_flat) !=
                                                   "Freq"],
                         names(.namesList)), character(0)))
    stop("names of `.strataCrossN_flat` and `namesList` must be identical",
         call. = FALSE)
  if (is.data.frame(.data) != TRUE)
    stop(".data must have class \"data.frame\"", call. = FALSE)
  
  ID_var_name <-
    names(.data)[grep("ID$", names(.data), ignore.case = TRUE)][[1]]
  if (length(ID_var_name) != 0)
    if (anyDuplicated(.data[[ID_var_name]])) {
      stop(c(
        "ID\u6709\u91cd\u8907: ",
        "\n",
        paste0(unique(.data[[ID_var_name]][duplicated(.data[[ID_var_name]])]),
               collapse = "\n")
      ),
      call. = FALSE)
    }
  ...
  if (anyDuplicated(vars))
    stop("duplicated '.vars'", call. = FALSE)
  ...
  for (i in seq_along(.namesList)) {
    filter_criteria <-
      lazyeval::interp(
        ~ as.character(which_column) %in% as.character(.namesList[[i]]),
  ...
  # sampleCross_flat 排序
  sampleCross_flat <-
    sampleCross_flat %>% arrange_(.dots = rev(.vars[var_names]))
  ...
  strataCompare <-
    cbind(
      .strataCrossN_flat[!(names(.strataCrossN_flat) %in% c("Freq"))],
      sample_size = n2,
      Completed_N = n1,
      diff = n1 - n2
    )
  ...
  names(strataCompare) <-
    ifelse(
      names(strataCompare) %in% var_names,
  ...
  ## sort variables
  strataCompare <-
    strataCompare[ifelse(names(strataCompare) %in% sort(paste(.vars[var_names], var_names , sep = "_")),
                        ...
                        pik = NULL)
{
  ...
  data.reordered <-
    arrange_(.data, .dots = rev(.vars[var_names])) ## 以後面變數為第一排序層級
...
write_output_xlsx <- function(ID_sample, .data, ID_var_name, fileName) {
  ...
  tempData <-
    tempData[order(match(tempData[[ID_var_name]], ID_sample)), ] # reorder by ID_sample
  ...
  wb <- openxlsx::createWorkbook(creator = paste0("EOLembrain", Sys.getenv("USERNAME"), sep =
                                                    "_"))
  ...
  openxlsx::writeData(
    wb,
    paste("N=", length(unique(ID_sample)), sep = ""),
    x = tempData,
    rowNames = FALSE,
    keepNA = FALSE
  )
  ...
  cat("exported to:\n", normalizePath(outputfileName, winslash = "/", mustWork = FALSE))
  invisible(NULL)
}
...
#' write_output_csv
#' @param ID_sample Character vector of IDs.
#' @param .data
#' @param ID_var_name
#'
#' @export
write_output_csv <- function(ID_sample, df, ID_var_name) {
  # ID_var_name: ID所在變數名
  # ID_sample: 要抽取的ID
  
  # .data排除重複id
  df <- df %>% distinct_(ID_var_name)
  ...
  missing_id <- setdiff(ID_sample, df[[ID_var_name]])
  if (!identical(length(missing_id), 0L)) {
    stop(c(
      "there are missing IDs: ",
      "\n",
  ...
  tempData <- df[df[[ID_var_name]] %in% ID_sample, ]
  tempData <-
    tempData[order(match(tempData[[ID_var_name]], ID_sample)), ] # reorder by ID_sample
  tempData_backup <- df[!(.data[[ID_var_name]] %in% ID_sample), ]
